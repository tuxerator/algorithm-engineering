  ...
  public static ArrayList<Vertex> read(String pathToFile) throws Exception {
    ...
    while ((line = reader.readLine()) != null) {
      if (line.charAt(0) == 'c') {
                ...
                if(!graph.get(Integer.parseInt(vIds[0])-1).neighbors.contains(ids[1]-1)){
                    if(ids[0]==ids[1]){ //case for self loop
                        graph.get(ids[0]-1).neighbors.add(ids[0]-1);  
                    } else {
                        graph.get(Integer.parseInt(vIds[0])-1).neighbors.add(ids[1]-1);
                        graph.get(Integer.parseInt(vIds[1])-1).neighbors.add(ids[0]-1);
                    }
                }
            }
        }
