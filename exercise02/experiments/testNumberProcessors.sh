...
for elems in ${@:2}; do
  ...
  timesQS=()
    ...
    for((j=1; j<=$1; j++)); do
      ...
      timesMS+=($(java -cp "../source/target/classes" "ae.sanowskiriesterer.ParallelMergesort" $k $elems))
      echo "Executing quick sort..."
      timesQS+=($(java -cp "../source/target/classes" "ae.sanowskiriesterer.ParallelQuickSortApp" $k $elems))
      printf "\033[3F\033[J"
    done
